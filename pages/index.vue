<template>
  ...
  >
    <!-- Scroll progress bar -->
    <div class="fixed top-0 left-0 z-[300] h-0.5 bg-[#e94560]" :style="{ width: scrollProgress + '%' }" />
    <!-- Decorative blobs -->
    ...
    <button
      ...
      :class="player.currentSound.value ? 'bottom-[140px] left-5 sm:bottom-5 sm:left-5' : 'bottom-5 left-5'"
      title="回到顶部"
      @click="scrollToTop"
    >
      ↑
    ...
    <button
      class="fixed z-[999] w-12 h-12 rounded-full bg-[#e94560] ring-4 ring-white/60 shadow-[0_4px_16px_rgba(233,69,96,0.5)] cursor-pointer text-white text-lg hover:scale-110 hover:shadow-[0_8px_28px_rgba(233,69,96,0.6)] transition-all duration-300 ease-spring flex items-center justify-center border-none"
      :class="player.currentSound.value ? 'bottom-[80px] left-5 sm:bottom-[80px] sm:left-5' : 'bottom-5 left-5'"
      title="反馈与建议"
