    ...
    <div class="max-w-7xl mx-auto px-5 pt-36 pb-24">
      ...
      <template v-else-if="err">
        <div class="text-center py-32">
          ...
          <p class="text-white/40 mb-6">数据加载失败</p>
      ...
      <template v-else>
        ...
        <div class="mb-24 sm:mb-32">
          <p class="text-[10px] sm:text-xs tracking-[0.25em] uppercase text-white/40 font-semibold mb-6">Statistics Overview</p>
        ...
        <template v-if="stats">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 lg:gap-6 mb-24 sm:mb-32" v-if="stats.hourly">
            <div class="lg:col-span-3 rounded-3xl border border-white/[0.06] p-6 sm:p-8" style="background: linear-gradient(180deg, rgba(255,255,255,0.02) 0%, rgba(255,255,255,0) 100%);">
              <div class="flex items-end justify-between mb-8">
                <p class="text-[10px] tracking-[0.15em] uppercase text-white/40 font-semibold">每小时趋势</p>
                <div class="flex items-center gap-4">
                  <span class="flex items-center gap-1.5 text-[10px] text-white/40"><span class="w-2 h-2 rounded-[2px] bg-[#e94560]" /> 浏览</span>
                  <span class="flex items-center gap-1.5 text-[10px] text-white/40"><span class="w-2 h-2 rounded-[2px] bg-white/25" /> 播放</span>
                </div>
              </div>
              ...
              <div class="flex justify-between mt-3 text-[10px] text-white/30 font-mono">
            ...
            <div class="lg:col-span-2 rounded-3xl border border-white/[0.06] p-6 sm:p-8 flex flex-col" style="background: linear-gradient(180deg, rgba(255,255,255,0.02) 0%, rgba(255,255,255,0) 100%);">
              <p class="text-[10px] tracking-[0.15em] uppercase text-white/40 font-semibold mb-6">今日事件</p>
          ...
          <!-- Top lists -->
          <div class="mb-24 sm:mb-32">
            <p class="text-[10px] sm:text-xs tracking-[0.25em] uppercase text-white/40 font-semibold mb-8">热门排行</p>
          ...
          <div v-if="stats.top_searches?.length" class="mb-24 sm:mb-32">
            <p class="text-[10px] sm:text-xs tracking-[0.25em] uppercase text-white/40 font-semibold mb-8">热门搜索</p>
            <div class="flex flex-wrap gap-2 sm:gap-3">
              <span v-for="(item, i) in stats.top_searches" :key="item.query" class="group inline-flex items-center gap-2 px-4 py-2.5 rounded-full border transition-all cursor-pointer" :class="i < 3 ? 'border-white/15 hover:border-white/25 bg-white/[0.04]' : 'border-white/[0.04] hover:border-white/10 bg-white/[0.01]'" @click="go('/?search=' + encodeURIComponent(item.query))">
                ...
                <span class="text-xs" :class="i < 3 ? 'text-white/65' : 'text-white/45'">{{ item.query }}</span>
                <span class="text-[10px] text-white/25 tabular-nums">{{ item.count }}</span>
              </span>
            </div>
          </div>
        </template>
